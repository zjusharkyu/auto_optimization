...
本项目深受 Andrej Karpathy 的 [autoresearch](https://github.com/karpathy/autoresearch) 启发，将其“AI 自动化科研迭代”的理念借鉴到了**C++ 极速代码优化**领域。

在这里，我们让 AI Agent（如 Claude 3.5 Sonnet / GPT-4o，运行在 Claude Code，Cursor 中）接管到一个极速订单簿撮合引擎项目中 [QuantCup 订单簿撮合引擎](https://github.com/ajtulloch/quantcup-orderbook)，通过无休止的 **“提出假设 ➡️ 修改代码 ➡️ 极限评测 ➡️ Git回滚/提交”** 闭环，在物理极限边缘疯狂试探，全自动榨干 CPU 的最后一丝性能。

## 💡 核心理念 (The Concept)

在高频交易中，几纳秒的延迟都关乎生死。传统的 C++ 性能优化极其枯燥：研发者需要手动尝试性能调优，然后跑分验证。

**如果把这个过程完全交给 AI 呢？**
我们为 AI 搭建了一个绝对公平且严苛的“黑箱沙盒”：
1. **单一输入源**：标准化的测试数据流 (`score_feed.h`)，严禁 AI 修改测试用例。
2. **稳定的评测栈**：定制的 `evaluate.py` 脚本，在 Linux 下利用 `os.sched_setaffinity` (CPU 核心隔离) 和 `SCHED_FIFO` (实时调度优先级) 消除操作系统噪音，提供极致稳定的性能中位数。
3. **迭代优化机制**：AI 运行脚本 -> 变快了就 `git commit` -> 报错或变慢了就无情 `git restore .`。

## 📈 优化成果展示 (The Results)

在30分钟时间里，AI Agent 独立用GLM 4.7 API完成了 20 次底层 C++ 代码迭代，其中成功 10 次。
...
- 订单簿引擎基座来自[QuantCup Orderbook](https://github.com/zjusharkyu/quantcup-orderbook) 的优秀开源实现。 
